...
const productSchema = z.object({
  ...
  price: z.coerce.number().int().nonnegative(), oldPrice: z.coerce.number().int().nonnegative(),
  accent: z.string(), accentColor: z.string(), image: z.string().trim().min(1),
  description: z.string().trim().min(1), highlights: z.array(z.string().trim().min(1)),
  groupType: z.enum(["Extintor", "Suporte", "Placa de Sinalização"]), type: z.literal("product"),
});
